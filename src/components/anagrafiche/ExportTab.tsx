...
export default function ExportTab() {
  ...
  async function confirmRestore() {
    ...
    try {
      ...
      await Promise.all(newFCs.map((f) => fundingCallsService.create({ code: f.code, name: f.name, body: f.body, deadline: f.deadline, notes: f.notes, probability: f.probability ?? 50 }, user.id)));

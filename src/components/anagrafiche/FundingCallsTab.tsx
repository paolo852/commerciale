...
interface FormState { code: string; name: string; body: string; deadline: string; lead_deadline: string; internal_deadline: string; description: string; notes: string; probability: number; }
const emptyForm: FormState = { code: '', name: '', body: '', deadline: '', lead_deadline: '', internal_deadline: '', description: '', notes: '', probability: 50 };
...
export default function FundingCallsTab() {
  ...
  function openEdit(fc: FundingCall) {
    setEditing(fc);
    setForm({ code: fc.code, name: fc.name, body: fc.body ?? '', deadline: toDateInputValue(fc.deadline), lead_deadline: toDateInputValue(fc.lead_deadline), internal_deadline: toDateInputValue(fc.internal_deadline), description: fc.description ?? '', notes: fc.notes ?? '', probability: fc.probability ?? 50 });
    setFormOpen(true);
  }
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    setSaving(true);
    try {
      const p = { code: form.code.trim(), name: form.name.trim(), body: form.body.trim() || null, deadline: form.deadline || null, lead_deadline: form.lead_deadline || null, internal_deadline: form.internal_deadline || null, description: form.description.trim() || null, notes: form.notes.trim() || null, probability: form.probability, source_url: editing?.source_url ?? null };
          ...
          <div className="grid grid-cols-3 gap-4">
            ...
            <div>
              <label className="block text-sm font-medium text-slate-700 mb-1.5">
                Scadenza interna lead
                <span className="ml-1.5 text-xs font-normal text-slate-400">(raccolta candidati)</span>
              </label>
              <input type="date" value={form.lead_deadline} onChange={(e) => setForm({ ...form, lead_deadline: e.target.value })} className={inputClass} />
            </div>
            <div>
              <label className="block text-sm font-medium text-slate-700 mb-1.5">
